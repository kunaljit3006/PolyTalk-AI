...
object PolyTalkApiClient {
    ...
    var baseUrl: String = "https://heykunal123-polytalk-backend.hf.space"
    ...
    suspend fun translate(text: String, fromLang: String, toLang: String): Result<String> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            var result = makeRequest(baseUrl, text, srcCode, tgtCode)
            if (result.isFailure) {
                // 2. Fallback to Localhost for development (Android emulator maps 10.0.2.2 to localhost)
                result = makeRequest("http://10.0.2.2:8000", text, srcCode, tgtCode)
            }
            result
        } catch (e: Exception) {
            Result.failure(e)
    ...
    private fun makeRequest(urlStr: String, text: String, srcCode: String, tgtCode: String): Result<String> {
